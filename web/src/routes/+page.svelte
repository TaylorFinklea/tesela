    ...
    {:else if notes}
      <ul class="px-4 py-2">
        {#each notes as note (note.id)}
          <li>
            <a href="/p/{encodeURIComponent(note.id)}" class="block px-4 py-3 rounded-lg hover:bg-accent transition-all">
              <div class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-3 min-w-0">
                  <span class="text-[14px] font-medium truncate">{note.title}</span>
                  {#if note.metadata.tags.length > 0}
                    <span class="text-[10px] px-2 py-0.5 rounded-full bg-primary/8 text-primary shrink-0">{note.metadata.tags[0]}</span>
                  {/if}
                </div>
                <span class="text-[11px] text-muted-foreground font-mono shrink-0">{formatTimestamp(note.modified_at)}</span>
              </div>
            </a>
          </li>
        {/each}
