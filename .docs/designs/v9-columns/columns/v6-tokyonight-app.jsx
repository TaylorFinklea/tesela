// === v6 — xplr Monolith. Tokyo Night theme. ===
...
function V6TNRail({ active, onPick }) {
  const groups = [
...
function V6TNListing({ data, selected, onSelect }) {
  if (!data) return null;
  if (data.kind === "blocks") {
    return (
      <div className="v6-bl">
        {data.blocks.map(b => (
          <div key={b.id} className={`blk indent-${b.indent} k-${b.kind} ${b.id===selected?"selected":""}`} onClick={()=>onSelect(b.id)}>
            ...
            <span>{b.kind!=="note" && <span className="ctype">{b.kind}</span>}{v6tnRich(b.text)}</span>
          </div>
        ))}
      </div>
    );
...
function V6TNBottom({ focus, tab, setTab }) {
  const tabs = [
...
function V6TN() {
  ...
  return (
    <div className={`v6 ${bottomOpen?"with-bottom":""}`} data-screen-label="01 Columns xplr — Tokyo Night">
      ...
      <V6TNRail active={widget} onPick={(id)=>{setWidget(id); setSelected(null);}}/>
      ...
      <div className="v6-middle">
        ...
        <div className="v6-pane-body">
          <V6TNListing data={listing} selected={selected} onSelect={setSelected}/>
        </div>
      </div>

      <div className="v6-focus">
        <V6TNFocus focus={focus}/>
      </div>

      {bottomOpen && <V6TNBottom focus={focus} tab={bottomTab} setTab={setBottomTab}/>}
