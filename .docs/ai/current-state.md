...
- **Net:** Same-LINE simultaneous collaborative typing now WORKS web↔web (verified two tabs) and web→iOS (verified sim, focused editor live-merges with caret remap). iOS→web outbound was device-verified earlier + is code-unchanged. The full bidirectional same-line test on two physical devices is the remaining user-driven check. All DIAG removed; clean `78c5fe0` build installed on the sim (launch with `-graphite`).
