...
- [x] Tasks 1-4: pure contract, ownership, engine relocation, recovery. Verify: `cargo test -p tesela-sync engine::loro_engine::tests::relocation`
- [x] Tasks 5-6: server route and web API/command contract. Verify: `cargo test -p tesela-server --test block_subtree_move && pnpm --dir web test:unit`
- [x] Task 7: ordered local-edit barrier plus pointer/keyboard UI. Verify: `cargo test -p tesela-server routes::ws::tests && pnpm --dir web check && pnpm --dir web test:unit`
