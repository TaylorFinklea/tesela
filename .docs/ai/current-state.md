...
- [~] **B-impl-4b RUNNING — HEAD-TO-HEAD** (wave9, bg): **Sonnet 4.6 vs kimi-k2.7-code**, same prompt (migrate widget/property/`/p`/`/s`/hoisted + DELETE the applySlash switch under grep gate). `.bench/wave9/`. Review both → merge the safe winner (careful: switch deletion is high-stakes; if neither is clean, do it myself) → score both.
