...
**Phase 1a DONE (`e936f56`): the relay is now a durable encrypted backup + restore source.** Removed the ack-triggered eviction (`gc_fully_acked_ops`) — the relay RETAINS the full encrypted op log (zero-knowledge unchanged). New e2e test: a brand-new EMPTY engine restores the WHOLE mosaic (3 notes, byte-identical) from the relay via `since=0`. Flipped 2 eviction tests → durable-retention. `cargo test -p tesela-relay` green (18). **NEXT: Phase 1b — snapshot deposit + snapshot-gated compaction (bounds the log; revisit per-note vs whole-mosaic snapshot + the note-id metadata-leak tradeoff); then Phase 2 = CF deploy.** LAN P2P track (transport router + mDNS + direct peer over Loro) needs its own grounding pass before building.
