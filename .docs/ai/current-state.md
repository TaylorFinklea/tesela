...
- [ ] **Sync durability P3 — remaining** (#72): (a) **iOS token-POST** — wire `AppDelegate.deviceTokenHex` → MAC-signed `POST /devices` (relay endpoint is live + waiting). (b) **⚠ TAYLOR DEPS:** enable Push for App ID `app.tesela.ios` + `aps-environment` entitlement; APNs `.p8` key → CF Worker secrets `APNS_KEY_P8/KEY_ID/TEAM_ID/BUNDLE_ID`. (c) HA-relay parity deferred (CF is canonical). Detail in `phases/2026-06-18-sync-durability-spec.md`.
