...
- [ ] **L3 KB2-4 NEXT** (per spec): **KB2** (senior·L, HIGHEST RISK — invert `GraphiteShell.onKey` ⌘-ladder; **reconcile the divergent `peek`/`command-station` runs FIRST**, commands.ts:633/655; read map at call-time; Opus DevTools QA — do NOT blind-swap) → **KB3** (leader/badges) → **KB4** (settings UI).
