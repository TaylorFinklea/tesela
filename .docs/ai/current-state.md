...
- C2.0 de-risk (`92a8e52`) ✓ · C2.1 TLR2 codec + binary WS (`f20f3f5`) ✓ · C2.2 per-note Loro doc + bootstrap + inbound apply (`68f8100`) ✓ — **web doc CONVERGES with the server over the live delta channel** (converge-check PASS; `blake3(slug)[..16]` confirmed on the wire; `NoteDoc.open/applyInbound/blockTextByBid/subscribe`; `@noble/hashes` added; 226 tests green; SSR clean). · C2.3 (bind CM6 ↔ `text_seq` bidirectionally: write = `update.changes`→`convertPos`→splice→`sendBinary`; read = remote text events→minimal CM transactions w/ auto cursor-remap; stop whole-text HTTP for in-place text; own-echo flag; structural ops stay HTTP) IN PROGRESS — must verify two-browser + web↔iOS same-block interleave via Chrome DevTools/Playwright MCP.
