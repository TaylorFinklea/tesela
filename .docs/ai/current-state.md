...
- [?] P6 awaiting Taylor product verify: representative 19-heading/8-query sandbox + browser/native-shell QA + report/harness-deck pass; fresh real corpus awaits iCloud materialization · tier_floor: senior · complexity: M. Verify: `cargo test -p tesela-server --test import_product_test && test -s .docs/ai/phases/2026-07-11-nonbullet-canonical-lift-report.md`

## Blockers
- iCloud FileProvider hangs enumerating the relocated Logseq container; source graph and live mosaic remain read-only.
