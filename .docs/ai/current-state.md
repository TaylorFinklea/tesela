...
- [~] **L3 KB3-4 RUNNING** (parallel Sonnet Agents, disjoint files): KB3 (`effectiveChord` in leader-tree + effective badges in palette/colon) · KB4 (settings UI: registry-driven rebindable section w/ capture/reset/conflict). Review + live-QA both → merge.
