...
- [ ] **B-impl-4b NEXT** (intricate finale): migrate widget + property + `/p` submenu + `/s` status + hoisted props, then DELETE the applySlash switch under the grep gate. High-stakes — fresh focus + careful review. **ROUTING (2026-06-13): gpt-5.5 OUT OF LIMITS → Sonnet 4.6 (`pi --model openrouter/anthropic/claude-sonnet-4.6`, auth confirmed) or opencode; minimax flaky under load.**
