...
- **kimi finding:** replies to plain prompts but does NOT agentically edit files via `opencode-go/kimi-k2.7-code`+pi (tool-integration gap). Untested: `openrouter/moonshotai/kimi-k2.7-code` (would isolate model vs tooling) — try that next if more kimi data wanted.
