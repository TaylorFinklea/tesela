...
- [?] Taylor verifies installed placement follow-up: nested block → blank day space promotes to root; block → nested row body becomes grandchild; no false descendant toast

## Blockers
- Human: physical `/Applications/Tesela.app` root-promotion + nested-target drag, then relaunch persistence.
