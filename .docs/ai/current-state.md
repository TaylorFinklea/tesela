...
### Server→web hub path PROVEN live (Playwright, 2026-05-31)
Loaded `/g`, opened a 2nd WS, PUT a marker edit via the API (simulating a remote/phone edit): `wsConnected:true`, `putStatus:200`, persisted:true, **`sawBinaryFrame:true`** (Phase A `ws_delta_tx` binary Loro delta broadcast on the edit — the bytes the phone consumes), **`gotNoteUpdated:true`** (finding-#4 web-invalidation `WsEvent` fired with the marker). `eventKinds:["binary","note_updated",...]` — both protocol halves live on one edit. Marker block cleaned up after. So the Mac-hub→web direction of the milestone works end-to-end; only the phone leg is unverified (device test).
