...
- [~] **Wave 10 RUNNING** (bg): ED1-fix 3-way — Qwen 3.7 Max (opencode-go) + Sonnet 4.6 (openrouter) + kimi-k2.7 (openrouter). GFM pipe-table render via StateField (NOT ViewPlugin — the runtime bug the prior attempt shipped). Review → runtime-QA the winner (real gate: unfocused table renders, no console error) → merge → score all 3.
