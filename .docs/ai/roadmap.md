...
12.2.x — Recurrence Completeness ✅ engine shipped (2026-05-22). BYDAY sets (`every mon, wed, fri`), `until` / `count` end conditions, skip-occurrence (`recur-bump` `mode: skip`), recurring on `scheduled::` (multi-field anchor), and the `weekends` keyword all land in `tesela_core::recurrence` + the server bump path + the EventKit round-trip. Spec/plan: `.docs/ai/phases/2026-05-21-recurrence-completeness-{design,engine-plan}.md`. **Still pending:** the web `DatePicker` / `date-parser.ts` recurrence UI + iOS recurrence display — a separate clients follow-up plan; the `recurring::` text grammar is fully usable without it.
