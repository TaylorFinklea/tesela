...
2. **Native desktop app.** ✅ **DECISION LOCKED: Tauri-wrap `/g`** (step 3 feature work then SHARED by web+desktop), NOT a fresh SwiftUI Mac app. Reasoning in `decisions.md` 2026-06-04. **MVP WORKING (2026-06-04):** native window + a child `tesela-server` bound to loopback serving the API + the static `/g` UI same-origin; verified on the real 100-note mosaic (UI + live WS + Loro collab). Single-writer `flock` enforced (the data-safety invariant), parent-death watchdog, embed is a loopback Loro-replica node (relay/peer-sync/mDNS off). Spec: `phases/2026-06-03-tauri-desktop-spec.md`. **tesela-ejn.1 wired (2026-07-01):** `tauri-plugin-updater` (startup + View > Check for Updates…, GitHub-Releases-hosted `latest.json`). **Desktop 0.1.2 bridge SHIPPED 2026-07-16:** clean-source tag `d0ddac68`; GitHub `v0.1.2`; Apple notarization accepted (`2e0fb2e1-6a84-49dc-aae4-3f0ebd61f225`); Developer-ID-signed/stapled app installed in `/Applications`. Replacement updater + Apple release secrets live in shared BWS project `finklea-dev`; the installed binary embeds the replacement trust root, and the published manifest/tar signature verifies against it. The reusable Developer ID identity was removed from the login Keychain after the BWS PKCS#12 signing proof (tesela-a5j). **⚠ Published 0.1.2 desktop assets are not self-contained:** after their disposable build checkout was removed, `/g` returned 404 and WebKit showed a white window. `tesela-1zn7` repairs packaging locally; a corrected release remains unpublished.
...
> **▶▶▶ ADVERSARIAL AUDIT (2026-07-14) — 19 new beads; ONE P0.** Report: [`phases/2026-07-14-adversarial-audit-report.md`](phases/2026-07-14-adversarial-audit-report.md). Fleet: Sol (architecture) · Terra (sync) · Qwen ×3 (core/iOS/web) · Opus (relocation) · Sonnet (server/verify/product). Every finding verified against source; 3 confident findings REFUTED before filing.
> - **🔴 tesela-73b (P0) — FIX FIRST.** Relocation boot recovery is fail-CLOSED: one unrecoverable intent makes the engine **refuse to open, permanently** (`loro_engine.rs:586` `?`-propagates into engine open; `relocated_bids_have_only_allowed_owners` returns `false` for a bid owned by *nobody*). On iOS = delete the app, lose unsynced data. It is the ONLY fail-closed path in an engine that fails soft everywhere else (poison probe skips; import plan degrades). ⚠️ **Landmine: a failed move + a later block deletion is enough to trigger it — and this code is live in the build being daily-driven.** Zero test coverage of an unrecoverable boot.
> - **[x] Relay durability cluster fixed 2026-07-15.** `tesela-gqd`: Rust + Worker rows reject lower per-entry snapshot sequences while inert chunks retain real row recency; upgraded requests declare sequence semantics, and unmarked legacy requests stay accepted but GC-inert. `tesela-fdd`: marked snapshots carry a mandatory authenticated group/stream/writer-seq routing record while legacy rows remain readable. `tesela-zip`: exact retained note+seq ops replay before snapshot fallback without moving the main cursor. Rust/Worker conformance passed 29/29; signed desktop + native iOS Simulator two-way/relaunch QA passed. Report: [`phases/2026-07-15-relay-durability-train-report.md`](phases/2026-07-15-relay-durability-train-report.md).
> - **Relocation has two more live bugs.** `tesela-9ut` (splice guard is **dead code**: `isReserved(slug)` against a barrier keyed by note UUID ⇒ keystrokes splice the doc mid-move) compounding with `tesela-flr` (retry/boot-recovery re-authors the destination from the captured snapshot, **destroying that keystroke**). Plus `tesela-2qh` (concurrent cross-note moves duplicate the subtree — cross-note relocation forfeits CRDT move semantics and there is no cross-note analogue of twin-heal).
> - **iOS**: `tesela-h8m` (tick timeout wedges sync — the abandoned tick keeps the admission lease and nothing can preempt it; also blocks mosaic switching) · `tesela-507` (APNs catch-up has no timeout ⇒ completionHandler never fires ⇒ **iOS revokes silent pushes for the install**) · `tesela-v1k` (triage writes can land on the wrong block) · `tesela-061` (agenda/query/search do a synchronous whole-corpus walk on the MainActor).
> - **Tasks**: `tesela-d33` (recurring-task roll silently dropped on write failure — while the identical call 80 lines away is fatal-and-regression-tested) · `tesela-g6i` (deadline reminders are fired-and-forgotten if no client is connected at that exact tick).
> - **RTC (`tesela-680`)** — Taylor: "full build in scope". Sol's verdict: **right about product priority, wrong only if that means default-ON before the gates.** Encoded as `tesela-hx8` (production-enable gate: kill switch + 6-rung rollout ladder + 72h physical 3-device soak) and the two XL preconditions `tesela-d0e` (executable relay sequence/snapshot/compaction model) + `tesela-q3p` (one per-note mutation sequencer), both wired `blocked-by` on 680.5. **Safe to start now: 680.1, 680.3, 680.4.** 680.8 must follow the durable-append contract — optimizing broadcast before defining commit order is backwards.
> - **Do NOT open a broad error-handling epic.** The server's alarming `unwrap`/`panic` counts are *test code*; production sync paths are genuinely well-hardened. The audit prevented that work.
> - **[x] Taylor's calls (2026-07-14):** v1 = perfected personal daily driver · harden desktop + iOS equally · RTC **full build, dark-ship gate ACCEPTED** · widgets = **both, in-app first** · properties = **all four gaps bite** (relations+backlinks · per-property color+icon · views/sets/kanban · schema UX) · **P0 fixed first, then device-test**.
> - **[x] tesela-73b FIXED + committed `afae5d02`.** Fail-soft at all **four** fail-closed sites — the fix found three MORE than the original report: an *undecodable* record also bricked boot, so a plain crash/power-loss mid-write was enough (no failed move, no deletion needed). Bad records now quarantine to `_relocations/failed/`. 292 sync + 107 server tests green. **[?] Taylor: physically verify Safari + installed desktop pointer drag from `ef750d55`, then relaunch for persistence.**

> **▶▶ 3-WEEK PLAN (2026-07-14) — the correctness floor, then RTC dark, then depth.** Sequenced with Sol; capacity assumption: one Lead + 2 expensive lanes + 3 S/M lanes. If capacity is lower, **cut week-3 breadth, never the gates.**
>
> **Week 1 — establish the correctness floor** (nothing here is optional; everything else is blocked on it)
> - **[x] Relay durability cluster:** `tesela-gqd` + `tesela-fdd` + `tesela-zip` completed 2026-07-15 with shared Rust/Worker conformance and installed desktop+iOS Simulator product QA. Report: [`phases/2026-07-15-relay-durability-train-report.md`](phases/2026-07-15-relay-durability-train-report.md).
> - Relocation: **`tesela-9ut`** (dead splice guard — one-line domain fix + audit every reserve/isReserved pair) then **`tesela-flr`** (stop re-authoring an edited destination) and **`tesela-2qh`** (cross-note move duplication — needs the deterministic tie-break twins already has).
> - iOS: **`tesela-h8m`** (generation fence + lease preemption — `tesela-dnl` alone is *insufficient*) and **`tesela-507`** (bound the APNs catch-up; an unbounded hang costs silent-push delivery for the life of the install).
> - CI: **`tesela-6hu`** — iOS must build on every PR. Half the daily-driver surface is currently never compiled in CI.
> - Continue **`tesela-myh`** (now with the second ungated writer in its acceptance).
> - **Exit gate:** gqd/fdd fixed · no stale iOS tick can commit · CI green *including iOS* · RTC durable-log semantics accepted · **zero RTC data applied anywhere**.
>
> **Week 2 — RTC dark + the iOS correctness that reads as data loss**
> - RTC lane: **`680.1`** (DO design, carrying the invariants) · **`680.3`** (latency harness — measure key→paint, commit→append, append→apply, apply→render, reconnect *separately*; one end-to-end number would hide the 500ms save path) · **`680.5`** behind the flag, blocked-by **`tesela-d0e`** + **`tesela-gqd`** · **`680.8`** *after* the durable-append contract (optimizing broadcast before defining commit order is backwards) · **`680.6`** continuously, not as a cleanup phase.
> - Preconditions in parallel: **`tesela-d0e`** (executable compaction/sequence model) · **`tesela-q3p`** (one per-note mutation sequencer).
> - iOS correctness: **`tesela-nnm.4`** (a synced-but-unopened note is invisible to search/query/agenda — that is *perceived data loss*, not a parity nit) · **`tesela-v1k`** (triage writes landing on the wrong block) · **`tesela-061`** (main-actor corpus walk).
> - **Exit gate:** RTC runs against synthetic groups and stays OFF for the real corpus unless dup/gap/reconnect/lost-receipt/snapshot/compaction tests pass, poll recovery reconstructs identical semantic state, web save-path transition tests pass, and iOS crash/restart cursor tests pass.
>
> **Week 3 — soak, conditional rollout, then depth** (start the soak at the *end* of week 2 so it gets a full 72h)
> - **`tesela-hx8`** rollout ladder: synthetic → shadow socket (observe sequence parity, apply nothing) → WS into the normal inbox with polling audit → *non-primary* device → primary corpus only after a verified backup+restore rehearsal → default-on only after the human gate. **Any red invariant leaves RTC implemented but disabled — that is a successful outcome.**
> - Depth, only if the gates stay green: properties **`tesela-nbf`** (Node/relations — the biggest Anytype gap) + **`tesela-11v`** (type-aware editing; breaks the iOS read-only wall) · widgets **`tesela-tko` ✅ shipped 2026-07-18** (configurable web rail + iOS Dashboard; Query notes/saved views reconnected) + **`tesela-a4m.1` ✅ shipped 2026-07-19** (read-only Agenda + Inbox home-screen widgets; device-profile provisioning tracked by `tesela-a4m.2`) + **`tesela-4o6`** (rail keyboard nav — a north-star violation) · tasks **`tesela-d33`** + **`tesela-g6i`** · editor **`tesela-8zd.17`**, finish **`tesela-baa`**.
> - **Explicitly NOT in these weeks** (Sol, endorsed): PDF annotation (`idf`) · code-block execution (`csj`) · remaining OS-integration expansion (`3ym`/`2pi`; the first OS-widget slice shipped after the in-app rail via `tesela-a4m.1`) · second-user/LAN/WASM/public-release (`6yw`, `y70`, `frz`, `559`) · god-file refactors for their own sake · the 73MB RELEASE.md purge.

> **▶▶ DAILIES SUBTREE RELOCATION — SHIPPED + PHYSICALLY VERIFIED (2026-07-15).** **[x] iOS:** Taylor confirmed TestFlight 1.1 (79) Move to works. **[x] Desktop drag transport:** Taylor confirmed the Pointer Events/capture path in `ef750d55` moves a subtree. **[x] Multiline display follow-up:** Taylor confirmed `054ebeb4` fixed the phantom multiline children and stuck placement lines. **[x] Nested placement follow-up:** `8feb933b` makes blank day space an explicit root append target and reserves only narrow top/bottom row rails, so the broad body of an already nested row means inside. Taylor confirmed the installed desktop behavior works; `tesela-8ig` closed. Automated evidence: 16/16 relocation E2E, 978/978 unit, zero Svelte/type errors, production build, workspace build, and full workspace tests. Product report finalized: `~/.harness/reports/tesela/20260714T010835Z-dailies-subtree-relocation-product-test/`.

> **▶▶ CONVERGENCE FIX — SHIPPED.** Layer 2 shipped as deterministic keep-winner (tesela-y11, decisions.md 2026-07-01); residue repair shipped + live mosaics scanned clean (tesela-49d); pure max-TreeID follow-up approved by Taylor, tracked as tesela-fte (blocked on tesela-engc.1).

> **▶▶ NORTH STAR ARC — multi-device live presence + cursors (collab).** Spec: `phases/2026-06-27-multidevice-presence-spec.md` (researched 2026-06-27, 5-agent workflow, adversarial-verified). loro 1.13.6 gives BOTH primitives FREE (verified): `EphemeralStore` (LWW presence) + stable `Cursor` (op-anchored, survives concurrent edits) — neither in `tesela-sync-ffi` yet. Transport crux: WS has in-memory broadcast (desktop real-time ✅); the **CF relay is store-and-poll — no real-time broadcast**, so iOS-over-relay needs a NEW channel (recommend a **CF Durable-Object WebSocket** for ephemeral presence alongside the unchanged store-poll ops). **Gated on layer-2 convergence** (a cursor on a block whose twin gets tombstoned goes stale). Phases: 0=layer-2 foundation [x convergence confirmed, June 25/26 healed] · 1=FFI-wrap Cursor+EphemeralStore [x `b7d26e92` engine + `5b6a8bf3` FFI, fully tested; loro-internal dep added — EphemeralStore isn't in public loro] · 2=desktop presence over WS [x server `cce9afa7` + web `e8374836`; VERIFIED two-page Playwright (live `.cm-remote-cursor` over WS); web uses plain {bid,offset} + CodeMirror decoration auto-remap] · 3=iOS presence [#15 — THE PHYSICAL-IPHONE BOUNDARY; sim-prep autonomous (FFI publish/render, op-anchored loro Cursor since UITextView doesn't auto-remap), hub-mode WS → CF-DO WS; real verify needs iPhone+Mac] · 4=collab polish (selection ranges, peer name labels [web widget supports a name flag, unpopulated], follow, presence sidebar, Savanne).

> **▶▶ HISTORICAL (2026-06-24): build 46/47 device-test fix batch — RESOLVED.** Taylor device-tested builds 46/47 → 5 findings. **[1] iOS↔web sync drift/clobber (looked like DATA LOSS)** root-caused to a sync-LIVENESS bug, NOT push logic (edits do converge): `RelayTicker` backoff slept `2 * 2^min(errs,12)` ≈ 8192s ≈ 2.3h between ticks (every comment claimed ~60s), and `.active → start()` couldn't wake a loop parked in that sleep. FIXED `e6d1d83b` — cap backoff ≤60s via a pure, unit-tested `backoffSleepSeconds`; add `wake()` (reset+restart, immediate tick) on foreground in BOTH shells; `RelayBackoffTests` regression guard; full TeselaTests green. Diagnosed live against the desktop's CF-relay state (iPhone WAS on the same relay/group — inbound seq matched; edits converged ~2h late, then instantly on app reopen = the bug's signature). decisions.md 2026-06-24. **Build 48 → TestFlight** (sync liveness `e6d1d83b` + date chip `5c65e9d2`; full TeselaTests 272 green). **Then build 49** (`594b0403`): web→iOS delete-not-propagating root-caused via a sim repro (foreground path WORKS on 48; the gap was background APNs wake) → APNs token registration now relay-scoped (HA→CF migration left CF with no token). Full suite 276 green.
...
> - [x] **iOS: slash deep-filter parity** — SHIPPED (tesela-7yy, 2026-06-30): iOS slash menu now fuzzy-deep-filters (mirrors web's `flattenedSlashFilter`); `/p1` resolves the flat registry verb → `setProperty(priority,p1)` directly. Builds 64/66; 6 unit tests.
> - [x] **iOS: inline NLP not firing** — SHIPPED (tesela-gq4 + tesela-j7g + tesela-ug7, 2026-06-30/07-02/07-03): root-caused across builds 62-66 — NLP only existed on today's-daily blocks (now on pages + past-day + capture) AND the lift/highlight had no builtins registry fallback (now uses a Rust+FFI shared parser). Also tesela-b1s (semantic token colors) + tesela-pfix.3 (NLP-lift conformance fixture) + tesela-fr1 (recurring-pattern regression) closed the surrounding surface.
...
> **▶▶ HISTORICAL (2026-06-18): iOS editor sprint SHIPPED (TestFlight builds 21–28, all pushed + Opus-verified) — batch closed.** Marker unification; Enter indent-inherit + empty-outdent + insert-after-cursor; word-wrap (`sizeThatFits`); capture target-swatch menu; **`[[` / `#` / `/` inline autocomplete** on one trigger-detection framework (`EditorAutocomplete`/`LinkSuggest`; complete page+tag source via new FFI `index_entries()` over the Loro index — fixes the lazy-materialization gap); Graphite **Search view** (gpt-5.5 via pi). See git log + `decisions.md` 2026-06-18. **Also shipped:** arch-review eval (3 open-source reports → ~20% signal) + the C23 backup-restore guard + C19/C20/C21/C24/C6 hygiene batch. Subsequent: iOS #65 capture-sheet footer (tesela-eec) and #64 command palette (tesela-cib) both landed in 2026-07.
> - [x] **#64 iOS mobile command palette** — SHIPPED (tesela-cib, 2026-07-02): `CommandManifestSource` (bundled + best-effort `GET /commands` refresh); executor-map-or-hidden; searchable palette; 8 iOS commands mapped. ADR-4 satisfied — no hand-copied vocabulary.
...
**Bundled desktop app (no CLI required) — SHIPPED 2026-07 (tesela-ejn.1 + tesela-ejn.2 + tesela-ffy + tesela-a5j).** Tauri-wrap `/g` (decisions.md 2026-06-04) — native window + a child `tesela-server` bound to loopback serving the API + the static `/g` UI same-origin; verified on the real 100-note mosaic (UI + live WS + Loro collab). Single-writer `flock` enforced (the data-safety invariant), parent-death watchdog, embed is a loopback Loro-replica node. `tauri-plugin-updater` wired (startup + View > Check for Updates…, GitHub-Releases-hosted `latest.json`). The lost updater trust key was explicitly reset for the manual 0.1.2 bridge; `build-desktop.sh` and `desktop-release.sh` take the replacement updater key only from namespaced entries in shared BWS project `finklea-dev`. The real full path passed 2026-07-16: Bitwarden-only Developer ID import, hardened-runtime sign, Apple notarize/staple, final updater tar/sign, GitHub publication, manual 0.1.2 install, installed health/signature proof, and remote updater-channel verification. Mosaic-switch UX exposed the embedded flag on `GET /mosaics/current` (interim until tesela-mmos lands). Spec: `phases/2026-06-03-tauri-desktop-spec.md`.
...
**iOS on-device Parakeet inference via FluidAudio — SHIPPED 2026-07.** `TranscriptionCatalog` (app/Tesela-iOS/Sources/Data/TranscriptionCatalog.swift) now lists `parakeet-tdt-0.6b-v2` / `parakeet-tdt-0.6b-v3` / `parakeet-tdt-ctc-110m` with `inferenceSupported: true` + a `parakeetVersion` token (`v2` / `v3` / `tdtCtc110m`) routed to FluidAudio's `AsrModels.downloadAndLoad`. No `downloadURL` — FluidAudio downloads + caches the CoreML-converted model itself (~450MB on disk). Streaming variant (P3) still in flight as tesela-v5t.3 (FluidAudio `StreamingUnifiedAsrManager`); the non-streaming on-device path is the one referenced by this Later entry.
...
> HISTORICAL — the live backlog is beads (`bd ready`), 2026-06-30 onward. Items below predate the beads pilot; treat as archive.
...
- [x] **Add a notarized desktop ZIP release recipe.**
