...
> - Depth, only if the gates stay green: properties **`tesela-nbf`** (Node/relations — the biggest Anytype gap) + **`tesela-11v`** (type-aware editing; breaks the iOS read-only wall) · widgets **`tesela-tko` ✅ shipped 2026-07-18** (configurable web rail + iOS Dashboard; Query notes/saved views reconnected) + **`tesela-4o6`** (rail keyboard nav — a north-star violation) · tasks **`tesela-d33`** + **`tesela-g6i`** · editor **`tesela-8zd.17`**, finish **`tesela-baa`**.
> - **Explicitly NOT in these weeks** (Sol, endorsed): PDF annotation (`idf`) · code-block execution (`csj`) · OS-integration expansion (`3ym`/`2pi`/`q8i` — the OS-widget half of your "both" answer lands *after* the in-app rail) · second-user/LAN/WASM/public-release (`6yw`, `y70`, `frz`, `559`) · god-file refactors for their own sake · the 73MB RELEASE.md purge.
