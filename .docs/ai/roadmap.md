...
> - **[x] tesela-73b FIXED + committed `afae5d02`.** Fail-soft at all **four** fail-closed sites — the fix found three MORE than the original report: an *undecodable* record also bricked boot, so a plain crash/power-loss mid-write was enough (no failed move, no deletion needed). Bad records now quarantine to `_relocations/failed/`. 292 sync + 107 server tests green. **[ ] Taylor: the physical drag test is now unblocked.**
