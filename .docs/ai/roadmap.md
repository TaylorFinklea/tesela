...
- [ ] **Split + immediate merge-back orphans the absorbed block's server row.** Enter-split a block, then Backspace-merge it back within/after the 500ms debounce: `handleBackspaceMerge` (web/src/lib/components/BlockOutliner.svelte:~1557) checks `!isLocalOnlyId(current.id)` before emitting the absorbed-block delete op, but the just-split new block's editor id is still `:new-…` even AFTER its creating upsert batch has flushed — so no delete op is sent and the absorbed block's row survives on disk (duplicate "second half" block; the survivor also re-asserts merged text). **A/B-verified pre-existing** on the pre-B4 base build (commit `00be880`), byte-similar artifact both sides — NOT a B4 regression. **Scope:** track "has this client-minted bid round-tripped?" (e.g. consult the saver's flushed-bid set, or test `current.bid` against the last-known server body) instead of the id shape; emit the delete when the row exists server-side. **Files:** web/src/lib/components/BlockOutliner.svelte (handleBackspaceMerge), web/src/lib/block-ops.ts (mergeOpsForBackspace docs). **Verify:** new e2e: split mid-text → wait >600ms → Backspace col-0 merge → on-disk note has ONE block with the merged text, absorbed bid gone. Tier hint: Sonnet — single-file logic + one e2e.
