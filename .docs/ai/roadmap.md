...
> **▶▶ TOP / NEXT MAJOR (2026-06-26): disjoint-lineage convergence + the Loro richtext crash — THE remaining sync problem.** After 6 surface fixes (builds 48–52) + the crash containment (`cdb4a0ec`), actively-edited notes STILL drift between iOS and desktop. Root cause: blocks edited on both devices form DISJOINT LINEAGES (separate Loro histories for the same bid, from the broken-sync back-and-forth). Reconciling the twins (`peer_genuine_block_changes` fork.import) is the exact op that PANICS loro 1.12 richtext (`insert_elem_at_entity_index` OOB); the build-52 containment now SKIPS that frame → the twins never heal → **asymmetric, permanent divergence** (observed: block `019f047a` = desktop "Brook" merged iOS's "Bro" → "BrookBro", but iOS skips the desktop frame → stays "Bro"). decisions.md 2026-06-26. This is NOT a surface bug — it's the project's recurring disjoint-lineage residue (`project_multidevice_convergence`) **+ a third-party Loro library bug**. **Two candidate fixes, both deep + need full re-test (do FRESH, not rushed):** (a) **upgrade loro** (currently `loro = "1.10"` → 1.12.0 locked; newer may fix the richtext OOB → the existing twin-heal would then work); (b) **rework the twin-heal to reconcile text WITHOUT the crashing Loro merge** (text-level pick, no `fork.import` of the poison frame). Also: prevent disjoint lineages forming (the shared-base bootstrap isn't fully holding). ⚠ Don't rush — sync is finally mostly working; a botched loro upgrade could destabilize everything. Stopgap for stuck notes: they're throwaway test data — delete + recreate to verify cleanly-created notes converge.
