...
> **▶▶ DAILIES SUBTREE RELOCATION — BUILD 79 DEVICE TEST (2026-07-14).** Shipped on `main` at `85d914ed`: the desktop shell now delivers HTML drag events, and iOS `Move to...` routes through the durable Rust relocation engine with exact retry, group fencing, and profile-switch admission. `/Applications/Tesela.app` was rebuilt, installed, Apple Development signed, and signature-verified; TestFlight 1.1 (79) uploaded successfully and is processing. **[ ] Taylor verify:** click **Always Allow** on the desktop's existing `tesela-sync-group-key` Keychain prompt, drag a parent plus children to another day, then install build 79 and verify iOS Move to + Cancel + relaunch persistence. Product test: `~/.harness/reports/tesela/20260714T010835Z-dailies-subtree-relocation-product-test/`.
