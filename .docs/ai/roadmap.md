...
7. [~] **Phase 5** — engine-level DONE, live wiring is the destructive cutover. Relay broadcast cursor model proven (`d29e631`): `produce_relay_updates`/`apply_relay_updates` + per-note broadcast cursor; 3-engine convergence + steady-state test. Recon + plan: `phases/2026-05-28-loro-cutover-phase5-6-plan.md`. **Remaining (destructive + device-gated, fresh effort):** wire the broadcast model into the LIVE relay envelope (protocol version bump — changes the shared wire, breaks SqliteEngine dual-write until both ends migrate); LoroEngine authoritative materializer (write .md) behind `TESELA_LORO_AUTHORITATIVE`; FFI swap + iOS LoroEngine + snapshot persistence + device-id bridge; on-device cross-device test (the user's — needs the iPhone).
