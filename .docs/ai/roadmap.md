...
> **▶▶ DAILIES SUBTREE RELOCATION — DESKTOP HOTFIX DEVICE TEST (2026-07-14).** **[x] iOS:** Taylor confirmed TestFlight 1.1 (79) Move to works. **[x] Desktop drag transport:** Taylor confirmed the Pointer Events/capture path in `ef750d55` moves a subtree. **[x] Multiline display follow-up:** Taylor confirmed `054ebeb4` fixed the phantom multiline children and stuck placement lines. **Placement follow-up (2026-07-15):** blank day space is now an explicit root append target, and block-row hit testing reserves only narrow top/bottom rails so the row body—including an already nested row—means inside. Automated evidence: 16/16 relocation E2E, 978/978 unit, zero Svelte/type errors, production build, workspace build, and full workspace tests. **[ ] Taylor:** in the installed desktop app, promote a nested block through blank day space, then drop another block onto a nested row to create a grandchild; relaunch and confirm both persist. Product test: `~/.harness/reports/tesela/20260714T010835Z-dailies-subtree-relocation-product-test/`.
