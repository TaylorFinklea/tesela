...
> - **P1 — JQL-first authoring (web)** ← delivering now: `/query` inserts a JQL template, query blocks display + edit JQL, placeholder/examples in JQL. (Engine already parses it — verify end-to-end first.)
> - **P2 — iOS parity:** `LocalQueryEngine` uses only `parseSimpleDsl` today → port the full JQL grammar to Swift so JQL queries run on iPhone (3-way conformance fixture, like L5).
