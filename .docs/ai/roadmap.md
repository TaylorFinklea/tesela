...
> **▶▶ DAILIES SUBTREE RELOCATION — DESKTOP HOTFIX DEVICE TEST (2026-07-14).** **[x] iOS:** Taylor confirmed TestFlight 1.1 (79) Move to works. **[x] Desktop agent gate:** `267c0312` adds an exact `text/plain` move-id fallback for embedded WebKit, removes immediate custom-MIME readback cancellation, and prevents failed starts from latching the session; `9139e45d` repairs the local installer. The corrected bundle is merged, Apple Development signed, installed at `/Applications/Tesela.app`, relaunched, and live-health verified. Automated evidence: 13/13 relocation E2E, 976/976 unit, type check, and native desktop tests green. **[ ] Taylor:** drag a parent plus children to another day and verify persistence after relaunch. Product test: `~/.harness/reports/tesela/20260714T010835Z-dailies-subtree-relocation-product-test/`.
