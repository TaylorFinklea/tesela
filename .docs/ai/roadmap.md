...
>   - [ ] **Layer 2 — mergeable child containers** (the no-data-loss ROOT fix). Spec: `phases/2026-06-26-mergeable-containers-spec.md`. Swap the 7 `get_or_create_container` sites → `ensure_mergeable_*` so block text/props DON'T fork across peers (converge on concurrent first-write). ⚠ HARD: `ensure_mergeable_*` errors on existing non-mergeable containers → the whole fleet needs a MIGRATION (delete+recreate re-forks per device). Design the migration first; prototype on a real-mosaic copy. FRESH work, high risk. Until then: layer 1 converges via lossy dedup (fine for one-device-at-a-time; simultaneous same-block edits can lose one side).
