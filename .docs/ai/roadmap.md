...
> - **The relay is trusted more than the topology lock claims.** `tesela-fdd`: snapshots seal under a **group-only AAD** (stream_id never bound in), so a buggy/hostile relay can serve note A's snapshot as note B — the tag verifies and the client imports it. `tesela-zip`: the "recovery path of last resort" envelope is preserved from GC but **nothing can ever re-read it**; and `put_snapshots_chunked` silently *skips* oversized streams, so a big note + one transient failure = permanent per-note staleness. Both sit in the same code as **`tesela-gqd`** (raised P2→P1, Sol's #1 item) — do all three together.
...
> - Relay durability cluster, all in the same code: **`tesela-gqd`** (stale-snapshot UPSERT guard, both relays) + **`tesela-fdd`** (bind stream_id into the snapshot AAD) + **`tesela-zip`** (make the retained recovery envelope reachable, or stop pretending it is). ← *Sol's #1, Terra's two verified bugs.*
...
> **▶▶ DAILIES SUBTREE RELOCATION — DESKTOP HOTFIX DEVICE TEST (2026-07-14).** **[x] iOS:** Taylor confirmed TestFlight 1.1 (79) Move to works. **[x] Desktop drag transport:** Taylor confirmed the Pointer Events/capture path in `ef750d55` moves a subtree. **[x] Multiline display follow-up:** the raw note was never duplicated; tab-prefixed Markdown continuations were reparsed as phantom blocks, and append's null target highlighted every bidless phantom row. `054ebeb4` keeps those lines inside their owning block and requires real matching bids for placement styling. Merged to local `main`; Apple Development signed, installed at `/Applications/Tesela.app`, relaunched, and live-health/DOM verified (one 16-line Pi block, zero phantom rows or stuck placement lines). Automated evidence: 14/14 relocation E2E, 978/978 unit, zero type errors, production build, and exact Rust parser regression green. **[ ] Taylor:** confirm one multiline subtree, no stuck red lines, and persistence after relaunch. Product test: `~/.harness/reports/tesela/20260714T010835Z-dailies-subtree-relocation-product-test/`.
...
**Bundled desktop app (no CLI required) — SHIPPED 2026-07 (tesela-ejn.1 + tesela-ejn.2 + tesela-ffy).** Tauri-wrap `/g` (decisions.md 2026-06-04) — native window + a child `tesela-server` bound to loopback serving the API + the static `/g` UI same-origin; verified on the real 100-note mosaic (UI + live WS + Loro collab). Single-writer `flock` enforced (the data-safety invariant), parent-death watchdog, embed is a loopback Loro-replica node. `tauri-plugin-updater` wired (startup + View > Check for Updates…, GitHub-Releases-hosted `latest.json`, signing key in Keychain — decisions.md 2026-07-01c). `build-desktop.sh` mirrors `desktop-release.sh`'s Keychain updater-key fallback. `cargo build -p tesela-desktop` + `clippy -p tesela-desktop --no-deps -- -D warnings` both clean. Mosaic-switch UX exposed the embedded flag on `GET /mosaics/current` (interim until tesela-mmos lands). **Notarized ship + one real signed `desktop-release.sh` run** still pending tesela-a5j. Spec: `phases/2026-06-03-tauri-desktop-spec.md`.
