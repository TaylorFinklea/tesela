...
> - [ ] **NEXT — Leader→editor wiring** (Taylor decided "wire it", 2026-06-17). Make `Space → i` (insert) and `Space → p` (properties) run the editor verbs on the **focused block**. **This is a sub-project**, not a one-liner: the editor verbs are coupled to the slash-trigger flow (`replaceTrigger` whole-block-replace from frozen `before`/`after`), so it needs a trigger-less leader context built by the focused `BlockEditor` + an `editorFocused` presence signal + `tesela:run-editor-command` routing. Full design + per-verb browser-verification matrix: **`phases/2026-06-17-leader-editor-wiring-spec.md`**. `tier_floor: lead` (registry + editor-context design), `complexity: L`.
