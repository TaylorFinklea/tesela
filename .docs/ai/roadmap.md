...
**Bundled desktop app (no CLI required) — SHIPPED 2026-07 (tesela-ejn.1 + tesela-ejn.2 + tesela-ffy).** Tauri-wrap `/g` (decisions.md 2026-06-04) — native window + a child `tesela-server` bound to loopback serving the API + the static `/g` UI same-origin; verified on the real 100-note mosaic (UI + live WS + Loro collab). Single-writer `flock` enforced (the data-safety invariant), parent-death watchdog, embed is a loopback Loro-replica node. `tauri-plugin-updater` wired (startup + View > Check for Updates…, GitHub-Releases-hosted `latest.json`). The lost updater trust key was explicitly reset for the manual 0.1.2 bridge; `build-desktop.sh` and `desktop-release.sh` now take the replacement updater key only from namespaced entries in shared BWS project `finklea-dev`. `cargo build -p tesela-desktop` + `clippy -p tesela-desktop --no-deps -- -D warnings` both clean. Mosaic-switch UX exposed the embedded flag on `GET /mosaics/current` (interim until tesela-mmos lands). **Notarized ship + one real signed `desktop-release.sh` run** still pending tesela-a5j, blocked on signing team `K7CBQW6MPG` into Xcode and creating a local Developer ID Application identity. Spec: `phases/2026-06-03-tauri-desktop-spec.md`.
