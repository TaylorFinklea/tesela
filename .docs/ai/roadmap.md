...
> **▶▶ DAILIES SUBTREE RELOCATION — DESKTOP HOTFIX DEVICE TEST (2026-07-14).** **[x] iOS:** Taylor confirmed TestFlight 1.1 (79) Move to works. **Desktop:** installed build 79 still no-ops despite `.disable_drag_drop_handler()`. The follow-up on `fix/desktop-wkwebview-drag` adds an exact `text/plain` move-id fallback for embedded WebKit, removes immediate custom-MIME readback cancellation, and prevents failed starts from latching the session; 13/13 relocation E2E, 976/976 unit, type check, release build, and Apple Development signature are green. **[ ] Agent:** merge and install corrected bundle. **[ ] Taylor:** drag a parent plus children to another day and verify persistence after relaunch. Product test: `~/.harness/reports/tesela/20260714T010835Z-dailies-subtree-relocation-product-test/`.
