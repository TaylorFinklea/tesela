...
> - **P2 — iOS parity** 🔄 IN PROGRESS: spec `phases/2026-06-15-jql-ios-parity-spec.md`. **Phase A DONE `0d6b3f77`** — 41 JQL conformance cases added (OR/parens/IN/NOT IN/LIKE/BETWEEN/IS NULL/infix), validated Rust+web green; iOS RED on 12 (the TDD gate). **Phase B** (restructure `LocalQueryEngine` flat-AST → Rust `BoolExpr` tree + parser + matcher) dispatched to a background agent against the conformance gate; Opus reviews adversarially + verifies + ships.
