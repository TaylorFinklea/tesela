...
- Verify result:
  ...
  - `cargo clippy --workspace -- -D warnings` → green.
  - `cargo fmt --all --check` → green (one rustfmt pass applied; the only reformat was splitting the `pub use retention::{...}` line over multiple lines because adding three new exports pushed it past the 100-char limit).
- Notes: Three named `pub const`s (`DEFAULT_DAILY: usize = 7`, `DEFAULT_WEEKLY: usize = 4`, `DEFAULT_MONTHLY: usize = 6`) added at the top of `crates/tesela-backup/src/retention.rs`, with a doc comment explaining they are the agreed default cadence (7/4/6) and that the server-side `backup-scheduler` reads the same numbers from `TESELA_BACKUP_KEEP_DAILY` / `_WEEKLY` / `_MONTHLY` env vars and falls back to these when unset. `GfsPolicy::default()` now uses the named constants; the `lib.rs` `BackupOptions.retention` doc comment now references the named constants instead of repeating the 7/4/6 literals inline; the three constants are re-exported from the crate root. In `tests/retention.rs`, the two test cases that hard-coded `daily: 7, weekly: 4, monthly: 6` were updated to drive off the named constants: the first test now calls `prune_gfs(root, GfsPolicy::default(), false)` and iterates `0..DEFAULT_DAILY as i64` (the cast is required because `chrono::Duration::days` takes `i64`); the second test asserts the constants are non-zero and builds its policy from them. This keeps a future change to the default cadence from silently leaving the tests lagging behind production.
