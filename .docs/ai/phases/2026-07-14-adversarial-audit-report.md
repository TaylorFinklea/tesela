...
## Product depth: what "half-baked" and "not at parity" actually mean (Taylor confirmed all four properties axes bite; widgets = both, in-app first)

**Properties (epic `tesela-427`).** The CRDT storage is genuinely structured — a block's `props` LoroMap holds scalar | nested `LoroText` | nested `LoroList` per key (`prop_containers.rs:1-9`) — but almost none of it surfaces. `parse_scalar` (`property.rs:88-102`) differentiates only **Checkbox** and **Number**; the other **9 declared types fall into a `_ => PropScalar::Text` catch-all** (`:100`). *We declared a 12-type vocabulary and shipped 2.*
- **`tesela-nbf` (P1) — `Node` is a declared type with zero relation semantics.** No page picker, no clickable render, **not in the backlink index**, no relation predicate in JQL, and not even reachable from the type picker. This is exactly what makes Tesela's properties feel like Logseq `key:: value` strings instead of an Anytype relation graph.
- **`tesela-11v` (P1) — type-aware editing, and the iOS read-only wall.** MultiSelect is *comma-string surgery on top of a real LoroList*; Checkbox renders the literal text "true"; Url/Email/Phone are never clickable. And on iOS the property system is **read-only end to end** — `PropertyRegistry.swift:67-69` says so outright ("Read layer only… No editor/toolbar/chip UI consumes this yet"), `TagChip` has no tap gesture. On the phone the only way to set a property is hand-typing `key:: value`.
- **`tesela-hvy` (P2) — schema UX.** Property rename is **unsafe** (no cascade — only tags have one, `tag_rewrite.rs`), so renaming orphans every `key:: value`, `tag_properties` row, and `property_overrides` key corpus-wide. No global property-browse page. `required` is declared and never enforced; uniqueness doesn't exist; `TypeDefinition.color` is a dead knob (folded into `tesela-e9r`).
- *Parity-grade already, do not rebuild*: the `extends` + `property_overrides` resolution mirrored line-for-line across all three engines; per-choice colors; typed query comparison.

**Widgets (epic `tesela-a4m`).** There are **two** widget systems and the good one is orphaned.
- **`tesela-tko` (P1)** — the live rail (`GrRail.svelte`, the only chrome since B3 deleted v4/v5) is a hardcoded 5-section list whose **"Add widget" button is literally a `console.log` stub** (`:51-54`); iOS mirrors the same stub *by explicit design*. Meanwhile the mature **9-widget Query-note system** (`system_widgets.rs:30-121`) is kanban/table-capable and fully configurable — and no longer feeds the rail. *We built the good widget system and then hid it.* Per Sol: perfect **Agenda, Inbox, Sync Health** first; do not build a general widget SDK.
- **`tesela-4o6` (P1) — the rail has ZERO keyboard navigation** (no `onkeydown` anywhere in `GrRail.svelte`). In the app whose locked north star is "every action reachable without the mouse", the most-used chrome is click-only. Same class as the vim bypass: behavior dispatched outside the command registry.
- **`tesela-2np` (P2)** — iOS inline `query::` blocks are list-only while the Views tab got native kanban/table (links `8zd.16`); iOS never writes table column config (a deliberate v1 call — this bead is the *revisit*, not a bug).
