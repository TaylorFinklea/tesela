...
The garble/sync arc and the iOS NLP/capture arc. Non-obvious rationale:

- **Convergence = bootstrap-before-author, NOT a deterministic seed.** The block-text garble (`Bothnice onenice one`) was a device authoring a note on a fresh DISJOINT `LoroDoc` (`doc_for_note_mut` mints `LoroDoc::new()` with this device's peer); when the relay's authoritative copy of the same bid later merged, the disjoint lineages UNIONED. Two fixes were on the table: (A) bootstrap the relay's authoritative note as a shared base before the first `record_local`, or (B) a deterministic byte-identical seed (`views_upsert` pattern). **Chose A.** B regressed (left a stray empty placeholder bullet because no shipped client authors into the deterministic bid — iOS/parser stamp RANDOM bids) and is parked dead on `wip/concurrent-convergence-shared-base`. A is `bootstrap_note_if_needed` on EVERY server authoring entry (best-effort, deadlock-safe = never re-enter the relay-tick's `state.write()`, resident-gated, non-destructive). iOS already did this on every author seam; the DESKTOP server was the actual culprit.
- **The deposit-strand invariant: a dirty note must ALWAYS export to its CURRENT state.** Separate latent bug (since build 54): `export_doc_update` only snapshot-fell-back on an UNDECODABLE broadcast cursor, not on a decodable cursor that is AT-OR-AHEAD of `oplog_vv` (an authoritative import rebasing a note "backward" leaves the committed cursor ahead → `updates(ahead_vv)` = empty → ships a content-less frame forever → `tick_outbound` sends 0, edits stranded). Fix: `vv.includes_vv(oplog_vv()) ⇒ Snapshot`. Self-healing (next produce snapshots; the confirmed PUT rewrites a fresh cursor). Lesson: an empty incremental delta for a note the device still holds is a bug, not "caught up."
- **Inline NLP is TAG-GATED on both platforms (web parity) → the capture type-picker is the gateway, not optional.** `InlineNLP.detect` returns nothing unless the block's tag declares the property (Priority/Deadline live in Task's `tag_properties`). So "type `p1 tomorrow` and it just works" requires the block to be `#Task` first. Capture had no way to tag → added a type/tag picker; picking a type tags the block AND runs add-time NLP. Lifted props are BAKED onto `Block.properties` at creation (the `#inbox` path), NOT a post-create `setBlockPropertyAndPush` (which would race the fire-and-forget write — the bid isn't recorded yet → `applied==0` no-op).
- **Capture and blocks share ONE registry resolver so NLP can't drift + works pre-sync.** `PropertyRegistry.effectiveLiftRegistry(live:forTags:)` returns the live registry when any tag is liftable, else `buildBuiltins()`. Without the builtins fallback, NLP silently no-ops while the Task tag page is synced but the Priority/Deadline Property pages aren't — capture hit this first; blocks had the same gap. Both now call the one helper.
- **Bare dates lift only when TRAILING.** A date phrase at the end of the text lifts the default date prop without a "due"/"by" intent word (matches how Taylor types "p1 tomorrow"); a date mid-prose still requires the intent word (over-parse guard). Slightly more aggressive than web by design.

## 2026-06-30 — iOS onboarding epic (tesela-mp0) re-scope + relay-attach fix

Picked up bd epic `tesela-mp0` (a Lead-escalation: "harden QR scan + RelayTicker race + add onboarding completion screen"). Used it to trial Sonnet 5 at Lead tier (see `~/.claude/model-scorecard.md`). Durable findings:

- **2 of the epic's 3 premises were STALE — verify the code, not the ticket.** The epic was paraphrased from a 2026-05-26 roadmap note. Reality: the QR-camera freeze was fixed 2026-05-20 (`f6393872` — `configureSession` runs off-main on a dedicated serial `sessionQueue`, by construction not timing), and the RelayTicker connect-vs-start race is contained (`0448ccf1` + `e6d1d83b`): `ensureCoordinator`'s nil-mosaic `guard` is a provably benign no-op (returns before any FFI/network call and before `tickOnce`'s catch arms, so it can't increment `consecutiveErrors`), and `connect(mosaic:)` resets errors + kicks an immediate tick the instant the mosaic is wired.
- **"Guarantee RelayTicker fully wired before the daily view becomes interactive" is WRONG for local-first — do NOT implement it.** `openEngineIfNeeded()` is network-free; iOS writes are durable to the engine + materialized files before any sync. Both shells deliberately render content immediately and wire the ticker async in `.task`. Blocking interactivity on sync would make every cold launch (esp. offline) feel broken for zero correctness benefit. The correct posture (already shipped) is render-immediately + surface readiness ambiently (`GrSettingsView` Sync row).
- **The real bug found while re-scoping: `PairDeviceView` misreported a relay pairing as disconnected.** `isAttachedToServer` recognized only `.http`, so after a `.relay` pairing (the loopback/Tauri-desktop-embed path — the primary onboarding route) it rendered "Not connected to a server" at the moment pairing succeeded. Fixed (`85dcf0a8`, bd `tesela-mp0.1`) with a pure static 3-way `displayState(for: Backend)` → `httpAttached`/`relayAttached`/`unattached`. **`refreshCode()` stays `.http`-ONLY on purpose**: a relay-only node has no Mac HTTP to issue a shareable QR — widening that gate would make relay mode hit an unreachable loopback. So only the displayed card + `introCopy` widened, never the fetch. Full `TeselaTests` 315/0.
- **Decomposed into children mp0.1 (done) → mp0.2 (onboarding "you're synced" screen) + the bugs the fact-check surfaced: mp0.3 (short-code pairing dead-on-arrival — `PairWithShortCodeView.submit` requires `.http` but fresh installs default to `.mock`; needs a small design call on which server a fresh short code resolves against) + mp0.4 (no camera-permission recheck on return from Settings via `fullScreenCover`) + mp0.5 (scanner-stall fallback nudge).**
- **Evidentiary correction (don't cite later):** the re-scope claimed `AppShell.swift:42-51` shows `connect()` before any `await` like `GrAppShell`; it's the REVERSE (`await activateMosaic` THEN `connect`). Harmless — `AppShell` is the `-legacy`-only shell and the race is the same benign no-op — but the citation is wrong if anyone revisits the race in the legacy shell.

### RESOLVED — the `task-tag-wip` git stash was stale, dropped
A `git stash` (`task-tag-wip`) held 34 files of WIP (+766/-4397): web v4/v5 chrome deletions, iOS feed-pagination + task-toggle persistence + `CaptureBar.pendingVoiceCapture`, Rust importer work. **Investigated 2026-06-30: stashed 2026-06-10 off `72c73783`, 439 commits behind HEAD; EVERY distinctive symbol (Rust importer fns, iOS `captureInsertIndex`/`taskStatusValue`/`persistTaskToggle`, the `MockMosaicServiceTests`) is already in HEAD, and the v4/v5 files it deletes are already gone — fully superseded, zero unique content.** Dropped (was `7472ab13`, recoverable via `git fsck --unreachable` for ~2 weeks if ever needed). No action left.
...
**Context.** y11 = "rebase relay snapshot-via-normal-tick instead of lossy dedup." Investigation (Opus; 5-agent map + direct reads + a characterization suite `crates/tesela-sync/tests/relay_inbound_rebase.rs`) found the CORE convergence was already handled by the 2026-06-29 work: `import_doc_update`/`apply_doc_update_status` ran a genuine-edit discriminator + force-heal. Empirically disjoint twins DID converge — but in TWO rounds with a transient split-brain (each replica adopted the OTHER's value; the shared-lineage LoroText merge settled it round 2). Taylor: do the durable 1-round fix, carefully. An independent adversarial review then caught HIGH-severity issues in the naive 1-round attempt; the shipped design below fixes them.

**Decision.** Resolve disjoint twins by KEEPING a deterministic WINNER NODE (never force-writing text), and use the SAME winner rule on EVERY apply path.

- **Keep the winner NODE, no text force-write.** The old heal force-spliced the winner's text onto the min-`TreeID` survivor; two replicas each doing that from their own base produced concurrent identical splices that INTERLEAVED next round → `betaBbetaB` doubling. Fix: `tombstone_twins_to_winners` keeps the chosen node (its text is already correct + identical on both replicas); `tombstone_duplicate_twins` cleans up leftovers; props still re-assert onto the survivor (`reassert_prop_heals`, idempotent union/LWW — safe across replicas).
- **Winner = max-`TreeID` among non-stale, non-empty tips** (fallback max-`TreeID` of all). MAX not MIN so a device's blind low-peer twin can't revert a peer's edit (the original lossy-dedup failure). Stale-guard made symmetric via per-node history (`fork_node_text_histories`): a tip whose text another twin's lineage authored-then-superseded is excluded (the "re-shipped old value" / Awesome→Awesome sweet incident) — handled regardless of peer ordering.
- **UNIFIED across paths (the key adversarial fix).** `import_authoritative_snapshot` now uses the SAME keep-winner resolution instead of `rebase_twins_onto_snapshot` (snapshot-origin). Formerly the WS path (TreeID axis) and the catch-up path (snapshot-origin axis) picked survivors on INCOMPATIBLE axes, so when the server's peer outranked the device's they tombstoned DIFFERENT survivors → **the block was LOST entirely** (adversarial repro #2, reproduced by `c13_ws_vs_authoritative...`: was `S="" D=""`, now converges). This was a LATENT total-loss in the pre-y11 shipped code too (min-`TreeID` vs snapshot-origin disagree when server-peer > device-peer); the unification fixes it.

**Removed (dead after the change):** `server_block_text_history`, `current_block_texts`, `rebase_twins_onto_snapshot`.

**Adversarial-review findings (independent agent, 2026-07-01):**
- #2 WS-vs-authoritative cross-tombstone total-loss → **FIXED** (unification) + regression test `c13_...`.
- #1 empty-twin split-delivery total-loss → **not reproduced** (`c12_...` converges; multi-round self-heal). The per-view modifiers (empty/stale) can still cost extra rounds on the incremental path but don't lose data.
- #3 stale-guard mis-flags a genuine tip (v1→v2→v1 vs independent v2) → CONFIRMED minor: converges but may pick the older value. Accepted; addressed by the follow-up.
- #4 plan/import not atomic + first-frame race → verify per-note apply serialization (follow-up bead).
- #5 `already_resident` gate → safe today (eager-load + empty-doc-for-nonresident); a data-loss HOLE only once lazy-load/evict (tesela-qql) lands — landmine on qql.

**Follow-up (filed, deferred): provably-convergent pure max-`TreeID`.** Reviewer's north-star: make the KEEP decision depend ONLY on the immutable `TreeID` (global-max always survives → provable convergence + perfect 1-round on the incremental path), letting emptiness/staleness influence nothing about which node is tombstoned. That drops the stale-guard + genuine-edit preference (a product change — the higher-`TreeID` twin's value wins a same-block conflict, not necessarily the newest) and is a broader change (flip global dedup to max, remove the discriminator's text logic) warranting device validation. The SHIPPED design keeps genuine-preference + stale-guard (closest to device-verified semantics) while fixing the total-loss + doubling + common-case split-brain; pure-max is the follow-up if perfect-1-round-on-incremental is wanted over genuine-preference.
...
12-reader map + 10-claim adversarial verify + parity audit (all evidence file:line-cited; findings report on harness-deck `tesela/20260701-arch-review`). Scope per Taylor: full-stack seams, M3 + next arc (~4–6 wk), decide-and-write authority, full backlog re-triage. Theme of the review: the architecture (Loro-as-truth, relay-as-mailbox, thin Tauri shell, conformance-gated relay pair) is sound; the risk is invariants enforced by N hand-synced copies instead of one owning artifact — the exact shape of every past data-loss bug (y11 cross-path survivor split, P1.13 editor class, iOS authoring no-op). Beads implementing these reference "per decisions.md 2026-07-01 ADR-n".

### ADR-1 — One apply orchestrator in the engine

`import_doc_update` / `apply_doc_update_status` / `import_authoritative_snapshot` are three hand-copied bodies of the same ~9-step apply sequence (loro_engine.rs:601-693, 715-772, 866-945). y11 unified the winner RULE, not the CODE — the structural bug-shape that produced y11 is still present. Decision: extract one private `apply_import(note_id, bytes, mode) -> ImportOutcome`; the three public methods become thin wrappers. Gate: `relay_inbound_rebase.rs` 13 cases + full 345-test suite, zero behavior change. Includes fixing the stale trait doc (engine/mod.rs:183-189 still describes pre-y11 "server-wins" semantics). Sequenced BEFORE lazy-load (ADR-6) so residency assumptions live in one place.
