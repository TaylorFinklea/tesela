...
### 2026-07-14 — Boot must never fail closed; and RTC ships dark behind a gate (audit-driven)

Two decisions out of the multi-model adversarial audit (report: `phases/2026-07-14-adversarial-audit-report.md`).

- **Engine open MUST fail soft. A stuck sub-system may never take the whole app down.** `recover_persisted_relocations` runs inside `LoroEngine::with_dirs` and `?`-propagated every failure, so ONE bad `_relocations/*.bin` meant the engine never opened again — and on iOS, where the record lives in the app sandbox, the only cure was deleting the app and losing every unsynced edit. Fixed `afae5d02`: four fail-closed sites now quarantine to `_relocations/failed/` and continue. **The generalizable rule (this is why it is an ADR, not just a bug):** every *other* risky path in this engine already fails soft on purpose — `probe_import_poison` SKIPS a panicking frame rather than aborting the process; `peer_import_plan` failure degrades to a raw import. Relocation recovery was the lone exception, and it was written that way *deliberately* (a durable intent "must" be honored). That instinct is wrong at boot: an abandoned move leaves a subtree duplicated or un-deleted — **visible, fixable, and strictly better than an app that will not start.** Any future durable-intent machinery (RTC pending ledgers, NoteDelete tombstones, key rotation) inherits this rule. Fixing it also proved the scope was wider than the report: an *undecodable* record bricked boot too, so a plain crash mid-write sufficed — no failed move, no deletion. **When a fix is "make X fail soft", grep every sibling `?` on that path before declaring it done.**

- **RTC (tesela-680) is built in full NOW, but ships DARK behind a production-enable gate (`tesela-hx8`).** Taylor's call was "full build in scope"; Sol's critique reframed it precisely: *right about product priority, wrong only if "full build now" means default-ON regardless of gates.* RTC does not repair durability — it adds a third delivery path on top of cursor and mutation boundaries the audit just proved fragile (the relay can cross-write notes via unauthenticated snapshot routing, `tesela-fdd`; a stale snapshot can overwrite a fresh one, `tesela-gqd`; the retained "last-resort" envelope is unreachable, `tesela-zip`). So: build the complete lane behind a kill switch; point it at the daily-driver corpus only after the veto conditions clear (gqd/fdd · iOS ticker generation fencing · the executable compaction model `tesela-d0e` · the per-note save-path ordering contract `tesela-q3p`), via a 6-rung ladder ending in a 72h physical three-device soak. **Any red invariant leaves RTC implemented but disabled — that is a successful outcome, not a failure.** Corollary locked: `680.8` (broadcast critical path) must follow the durable-append contract — optimizing broadcast before defining commit order is backwards.
